...
2. Apply the migration in an approved rollout window. Confirm every cockpit/media table has RLS enabled; `anon` and `authenticated` have no privileges; only `service_role` has table/function access; and `service_role` can use the atomic site-proposal publication function.
...
4. Create an owner-approved internal campaign with non-sensitive media. Confirm ID-based private preview rejects unauthorized users, owner approval is retry-safe, one reused media item produces one public derivative, and the public derivative is stable over HTTPS.

...
| Billing | Stripe and Clerk state can diverge or process duplicate/out-of-order events | P1 | Provider event proof pending | Durable ordering/deduplication is released. Live reconciliation reports zero subscriptions missing ledger coverage, and Supabase contains one subscription baseline, but the billing-event ledger still contains zero Stripe events. Reconciliation also reports one existing subscription whose stored Clerk user cannot be found. Have that customer sign in or register with the Stripe email, then resend the same real Stripe event twice and verify one ledger row plus an unchanged entitlement before closing. |
...
- Current billing proof: one subscription baseline row, zero subscriptions without a stored Clerk mapping, and zero billing-event rows. The provider event ledger therefore remains open until a real Stripe event is delivered and the duplicate resend is recorded.

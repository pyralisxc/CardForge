...
| Social publishing | A credential, wrong destination, scheduler retry, or partial approval could expose media or publish an unapproved post. | P1 | Open; hard-disabled. Keep extended-contribution/native Meta gates off until protected media, scoped developers, encrypted tokens, exact destinations, idempotent claim/retry, harmless publication, and durable delivery postflight in `docs/operations.md` pass. Communities remain manual-only. |
| Large local projects | Long artwork sessions can exceed browser quota or degrade recovery despite IndexedDB, actual-write failure reporting, snapshots, and project export/import. | P1 | Accepted. Browser capacity is not a CardForge allowance and is not proactively gated. Revisit after a real quota/recovery report or when durable cloud project storage is intentionally designed. |

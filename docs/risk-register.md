...
| Solo maintainer | A PR author cannot independently approve their own change, so one required approval would deadlock releases. | P2 | Accepted while `@pyralisxc` is the only trusted code owner. Keep resolved threads plus required CI/smoke checks; require one approval when a second trusted reviewer joins. |

...
3. `page.tsx` `useEffect` fires → `GET /api/templates` → `setDefaultTemplatesFromFiles()` and `mergeUserTemplatesFromFiles()`
...
Developer submissions move through `draft`, `submitted`, `voting`, `publish_candidate`, `published`, `archived`, or `rejected`, with submitted assets entering developer voting immediately. Published-to-site status is lifecycle state, not a library tier. Creator-facing visibility is intentionally only `free` or `paid`; internal `developer` and `hidden` tier values mean pipeline-only or not-live inventory, not extra customer library levels. Votes recalculate `qualityScore`, `calculatedAccessTier`, and `tierDecisionReason`, while owners can publish to Starter Library, publish to Creator Pass, keep assets pipeline-only, archive, recover, or reject. Only `published` submissions count toward monthly developer requirements and future contribution eligibility. Signed-in paid/free user uploads remain browser/project-local, normalize to `localOnly`, and are visibly separate from the site/developer submission pipeline; anonymous visitors can explore but must sign in before adding custom art to their local asset library.

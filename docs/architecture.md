...
Studio navigation has three product workspaces over the same project state. **Templates** owns reusable front Templates and separate card-back Templates. **Make Cards** owns active production for the currently selected set: its selected front Template/back, card values, generation, review, and export settings. **Sets** is the normal production library for browsing local sets, loading cloud mirrors into the local workspace, renaming/selecting sets, reviewing their cards, importing/exporting, backing up a set, and handing the selected set to Make Cards. Sets composes the existing `cardSets`, transfer, renderer, and cloud-mirror owners; it does not create another set registry. Account Home now uses the shared CardForge Environment shell and its compact zone rail, command band, object rows, detail inspector/mobile sheet, and boundary vocabulary. Its live projection resolves the active local Set first, then recent provider/temporary work, without letting unavailable account or provider state masquerade as Free, signed-out, disconnected, or missing work. The directly linkable Library, Storage & connections, billing, Profile, and Developer sections remain on their existing composition until each reaches action parity in the Environment shell. The account **Library** is a combined read model over device sets, cloud mirrors, connected projects/assets, local-folder bindings, and temporary working drafts. It merges proven duplicate locations but owns no bytes, sync protocol, or second content registry. The sibling **Storage & connections** section owns capacity, provider health, destination selection, permissions, and location-specific deletion. Generated cards own independent front `data` and back `backingData`; layouts remain reusable blueprints.

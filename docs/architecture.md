...
- `src/features/app-shell`: creator interaction sessions, feature-owned action execution, contextual-tool composition, compatibility Studio ingress, and workspace bootstrap.
- `src/features/home`: user-facing spatial Desk composition, exclusive Set and Artifact focus, contained-Artifact organization, culling/LOD, ordered non-visual navigation, and return context over the native project and Library owners. The historical internal feature id remains `home`.

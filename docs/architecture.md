...
`src/features/project/model/workLocations.ts` owns destination capabilities for device, Google Drive, and local folders; Library composes that native contract. Copy and Move are separate commitments: Copy leaves the source unchanged, while Move may remove the source only after the destination package has been written and verified. Unsupported provider-to-provider paths require opening a device copy first rather than pretending CardForge operates a universal sync layer. The preferred save destination only orders new destination choices; existing attachments retain their destination. Older attachment metadata without proven package scope must be reopened before overwrite.
...
- `src/features/card-rendering`: shared preview/rendering, rich text, vector shapes, thumbnails, appearance, watermarks, and the persistent Artifact scene. `ArtifactScene` owns one rendered node per visible card; `ArtifactSlot` supplies responsive layout geometry and accessible activation targets at stack, board, focus, and edit depth.
...
Set view derives a compact Template row from the front/back references of its cards. Solid-bordered Template controls show names and linked-card counts and open the exact Studio Template; generated-card scene borders use a matching stable accent and dashed style, with textual Template labels. This decoration stays outside canonical artwork/export rendering. Card previews fit their tile geometry with room for captions, without rewriting saved positions. Text/image editability is expressed through native field contracts and real canvas elements. Image fields retain generator-side fit/position/scale/rotation/offset/flip controls.
...
`cardforge_sync_contributor_asset_registry` is the single database publication owner for Templates, Styles, textures, dividers, icons, images, fonts, and Sets. It serializes changes by registry identity, preserves the active publication while a newer candidate is reviewed, and prevents older revisions from replacing or hiding it. Set packages retain their immutable source URL and revision metadata. Media finalization decodes raster pixels with Sharp and font tables/glyphs with Fontkit before registry admission; package finalization uses the canonical project decoder.

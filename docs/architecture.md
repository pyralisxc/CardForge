...
- `src/features/app-shell`: Studio Set Desk shell, contextual-tool composition, and workspace bootstrap.
...
- `src/features/card-generator`: Studio Set Desk composition, single/bulk card creation, native Set organization, image controls, portable Set transfer, and exports.
...
Studio has one active **Set Desk** over the project state. The selected Set remains the stable context while Template editing and generation open as focused tools; Save/Move, Output, and contributor Pipeline submission open as contextual sheets or provider dialogs and return to the same Set selection and organization. The old Templates / Make Cards / Sets peer navigation and persisted tab vocabulary are retired. The Set Desk composes the existing `cardSets`, organization, transfer, renderer, export, storage, and Pipeline owners; it does not create another Set registry, output path, or submission path. Template Editor still owns reusable front Templates and separate card-back Templates. Card Generator still owns single/bulk generation and rendered output. Project still owns Set identity, tags, grouping/sort, freeform positions, browser persistence, packages, folders, and Drive transfers. Pipeline still owns submission and publication. In current domain truth, `Set` is the one authored-work container. Provider/package surfaces may still call the same kind of object a project, but CardForge does not model a separate Project parent or a Project-to-Set registry. Account Desk, Library, and Profile use the shared CardForge Environment shell and its stable zone rail, compact command band, aligned object/setting rows, detail inspector or mobile sheet, and boundary vocabulary. Desk is a constrained spatial surface over meaningful local Sets plus recent provider or temporary work. It hides the untouched bootstrap Set, supports search/source filters/sort/pinning, uses the canonical authored-object renderer, and focuses one work container into contained-card organization without taking ownership from `project`. Set-owned organization travels inside the project document: a normalized tag catalog and layout policy live on the Set; cards reference stable tag ids and optional positions/change timestamps; Template fields remain derived facets rather than duplicated tags. Library uses the same preview owner and pools device, Google Drive, and attached-folder location records under the Set identity instead of presenting a second Project object. It remains a combined read model and owns no bytes, sync protocol, or second content registry. **Storage & connections** is a focused Library tool that composes browser, local-folder, Google Drive, personal-library, and temporary-draft lifecycle owners while keeping every default, save, transfer, reconnect, detach, and location-specific deletion scoped to the named source. **Profile** owns compact identity, security, access, personal utility, Contributor, and Owner entries. Clerk identity/security and Stripe billing open as focused Profile views and retain their provider-native controls, callback routes, and failure meanings. Contributor capabilities enhance Desk, Library, Studio, and Profile; Owner remains the separate permission-gated operational environment. Generated cards own independent front `data` and back `backingData`; layouts remain reusable blueprints.

...
| `/api/roadmap` | `GET`, `POST` | Reads the public Chronicle/feature board, creates compact signed-in suggestions, and lets dev accounts create official timeline items. |
| `/api/roadmap/items/[itemId]` | `DELETE` | Lets dev accounts delete/manage official roadmap items. |
...
| Table / RPC | Purpose |
| --- | --- |
| `cardforge_roadmap_items` | Public feature suggestions, official Chronicle items, level-up checkpoints, status, ordering, monthly costs, and archive state. |
...
Pipeline lifecycle terms are `draft`, `submitted`, `voting`, `publish_candidate`, `published`, `archived`, and `rejected`. Access tier is separate: `hidden`, `free`, `paid`, `developer`, or `official`.

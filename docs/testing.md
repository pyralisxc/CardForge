...
Tests that pin a component path, internal helper name, one-time migration shape, cutover state, or exact source composition should normally take the third path. Use `npm run test:inventory` when test growth itself is under review; the detailed analysis report stays opt-in. Its quiet `test:inventory:check` mode belongs to the final gate and rejects unclassified source readers or stale classifications. Any material increase should be explainable by product or production risk, and cleanup should retire probes made obsolete by broader coverage.

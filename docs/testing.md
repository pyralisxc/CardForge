...
Tests that pin a component path, internal helper name, one-time migration shape, cutover state, exact source composition, or a retired interaction mode should normally take the third path. Broad browser workflows may remain in `smoke:ui` when they still protect a real user contract, but they should not stay in the golden manifest merely because they once carried an `@golden` title. Use `npm run test:inventory` when test growth itself is under review; the detailed analysis report stays opt-in. Its quiet `test:inventory:check` mode belongs to the final gate and rejects unclassified source readers or stale classifications. Any material increase should be explainable by product or production risk, and cleanup should retire probes made obsolete by broader coverage.

`npm run verify:full` is the sole complete non-browser code-health gate. It owns lint, types, both Vitest lanes, quiet test-inventory enforcement, architecture, migration safety, and the production build exactly once; the required GitHub `verify` job is authoritative for the coherent candidate. Local agents normally stop at focused/affected evidence and use a local full run only for high-risk work or a local/CI discrepancy. CI keeps `npm run smoke:golden` as a separate, conditionally routed browser job so browser setup is skipped when no browser-owned surface changed and the full scale/soak suite remains selective.

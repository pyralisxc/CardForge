...
Google Drive preserves its last known same-account Library/Desk projection only for a transient unavailable read and distinguishes authentication, permission, quota/limit, conflict, not-found, and transient unavailability. An explicit authentication, authorization, or not-found result clears that protected projection promptly; it never carries results across an account scope. Native `nextPageToken` pages are followed by client discovery and provider-facing listing; only an OAuth `invalid_grant` persists reconnect-required state, while transient token refresh failures leave the existing connection recoverable. A Drive `webViewLink` remains a provider navigation destination; its separately returned, access-scoped `thumbnailLink` may be used only as an ephemeral Desk/Library visual reference. CardForge never treats a browser link as image data, downloads every package just to make a tile, or persists that thumbnail as an owned asset.

Connected storage has four explicit roles: the scoped browser workspace is the fast working and recovery copy; a versioned `.cardforge` package is the portable project; Google Drive or a chosen local folder may durably own project packages; and the personal Library indexes explicitly authorized reusable files while their provider retains the bytes. Account → Storage & Library owns provider setup and discovery. Portable personal storage is available to every signed-in Free or paid creator; CardForge never uses a plan to block the creator from keeping, reopening, copying, or moving work they own. Google Picker grants individual files or a destination folder through `drive.file`; a selected folder is not treated as recursive permission to every pre-existing child. Personal Library rows retain stable provider ids, exact revisions, optional Picker resource keys, MIME type, semantic role, size, content hash after reading, and verification timestamps. Shared Drive reads use the same provider capability boundary, and materialization rechecks metadata after downloading so changed bytes are never recorded under an older Drive revision. Generic images require an explicit role because their bytes cannot reliably distinguish artwork, icons, textures, dividers, or frames. CardForge materializes a provider asset only for preview, editing, export, packaging, or an agent checkout, and never exposes provider credentials to MCP tools.
...
- Google Drive uses a dedicated Preview Google Cloud project, OAuth Web client, Picker key, and token-encryption key. Production uses its own matching provider project and credentials. Each Picker key permits only its environment's canonical origin plus Google's Picker iframe host and remains restricted to Google Picker API; ordinary feature-deployment hosts are not authorized.
- Google OAuth connection establishes account authorization only; it never creates or selects a project folder for a new/different account. A same-account reconnect preserves the creator's previously selected folder. Google Picker remains the native browser selection surface. Project-folder selection opens at My Drive so the user can authorize a destination; the current CardForge folder is displayed as account storage state rather than used as an empty Picker parent. Google OAuth keeps random provider `state` authoritative while short-lived HttpOnly cookies preserve the server-sanitized exact initiating Library return and a one-way fingerprint of the initiating CardForge account; callback completion is rejected if the CardForge account changed, and every outcome clears the transaction cookies. Google Drive Set packages and explicitly indexed assets feed the account Library read model while Drive remains authoritative for their bytes, revisions, links, and permissions. CardForge writes `cardforgeWorkId` as private portable identity metadata. Library joins a working Set to an exact account/file binding; distinct provider files remain distinct even when their portable work ids match. Provider updates require the exact Drive and package revisions previously read.
...
**CardForge owns:** account-scoped namespace selection, first-account guest adoption before Desk/Library hydration, revisioned multi-tab writes with recoverable conflict guidance, project recovery, content-addressed Blob references, lazy mounted resolution, asset bounds, export/import portability, the default destination preference, and the copy-before-delete transfer commitment. Returning accounts always resume their own saved workspace; guest work remains separate for explicit backup/import, and unreadable account bytes are never replaced by sign-in. Only an account without a saved workspace automatically adopts the guest lane. After required artwork is copied, the first account workspace, catalogs, and exact guest-source removal commit atomically; concurrent changes reject adoption and preserve both sides. Browser package imports verify package hashes, persist each asset once as a scoped Blob, and give workspace and asset-catalog state durable references; mounted renderers, pickers, and font CSS acquire and revoke object URLs per consumer. Base64 remains a supported legacy/input representation and is externalized before normal persistence. Portable re-save resolves scoped Blob references through the same bounded package writer, so browser-only references never escape into `.cardforge`, Drive, or Pipeline files. CardForge exposes the browser persistence state and requests stronger eviction protection only from an explicit user action, while stating that persistence is not a backup. Those are product requirements that generic Zustand persistence does not define. CardForge intentionally does not invent a universal provider-sync layer; each supported provider earns a narrow native adapter and explicit capability contract. Google Drive uses native resumable upload and provider revisions; local folders and device-file handles use the browser File System Access lifecycle, external-revision preflight where relevant, streaming writes, and read-back verification. A known non-retryable Drive condition—missing document, dirty external revision, permission/authentication/not-found response, or a confirmed receipt awaiting link repair—pauses automatic Drive writes until the creator explicitly rechecks or repairs it; offline browser work waits for online reconciliation rather than polling. Complete-body browser/provider boundaries use the same bounded package writer's Blob form rather than a parallel encoder. Provider-to-provider transfer is offered only where those owners can be composed safely.

Browser capacity is not a CardForge allowance. The UI may show device usage in the storage/account lens, but normal local creation is not proactively gated by an estimated browser quota. Actual rejected writes, corrupt reads, and unsafe individual files remain explicit failures. `projectWorkspaceDocument.ts` applies imports to a detached workspace draft, then commits the workspace and readable merged asset/font catalogs in one native IndexedDB transaction. Account or concurrent workspace changes abort application. Restore enters the same serialized revision owner and advances its revision; invalid current bytes are retained with recoverable quarantine. Emergency editable backups and independent recovered-copy imports are available to everyone. Automatic Drive source deletion during a Move is disabled pending atomic provider proof.

Drive bindings distinguish provider account/file identity, portable entity identities, local working identities and baseline content revision. The provider revision token is `head:` plus SHA-256 of Drive’s native binary `headRevisionId`, fitting the existing 80-character persistence field. Drive’s numeric `version` also includes invisible metadata changes and is not a content preflight token; Google defines a [revision as a content change](https://developers.google.com/workspace/drive/api/guides/change-overview). Missing head identity remains unavailable; numeric legacy bindings require explicit clean Refresh or Save as new when locally changed. Exact native content identity and package hash checks remain preflight checks, not atomic compare-and-swap. Open resumes that binding; explicit Refresh refuses local changes; Copy detaches. Saves supply a canonical first-card PNG through Drive's native `contentHints.thumbnail`, and a private authorized endpoint delivers provider thumbnails without importing packages. Google owns resumable sessions, revisions and thumbnail storage. An interrupted upload first queries the resumable session and continues from Google's acknowledged byte range when possible. Only an expired/unreadable session or repeated inability to prove the final receipt remains an unknown commit outcome; CardForge never blindly replays that write. Metadata preflight alone is not an atomic compare-and-swap guarantee. [Google upload lifecycle](https://developers.google.com/workspace/drive/api/guides/manage-uploads), [native thumbnail guidance](https://developers.google.com/workspace/drive/api/guides/file-metadata).

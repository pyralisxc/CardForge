...
Google Drive preserves its last known same-account Library/Desk projection only when marked stale and distinguishes authentication, permission, quota/limit, conflict, not-found, and transient unavailability. Native `nextPageToken` pages are followed by client discovery and provider-facing listing; only an OAuth `invalid_grant` persists reconnect-required state, while transient token refresh and revoke failures leave the existing connection recoverable.

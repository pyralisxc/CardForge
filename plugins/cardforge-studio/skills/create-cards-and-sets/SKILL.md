...
5. Call `preview_card_set` after meaningful changes. Inspect both the structural artwork diagnostics and the native contact sheet composed only from canonical CardForge-rendered cards shown in chat. The sheet is composed from native CardForge-rendered representative cards; it is not a separate Template interpretation.

...
- Studio: `/studio`
- Account and subscription access: `/account`
...
npm run architecture:check # Ownership and retired-lane guard
...
- `supabase/migrations/`: Ordered database migrations for shared product and operational state.
...
Stripe Checkout owns both payment lanes, but their metadata and entitlement behavior are separate. `product_access` uses the authenticated Creator Pass Price and may update CardForge entitlement. `creator_support` offers a server-bounded customer-selected one-time amount plus fixed $1, $5, $10, and $20 monthly Prices, may be used without a CardForge account, and never updates product entitlement. The server verifies each purpose, amount, currency, recurrence, and monthly Price before checkout or webhook processing. Follow the [billing reconciliation runbook](docs/operations.md#billing-reconciliation) after billing-purpose, webhook, or provider-identity changes.
...
- [docs/developer-cockpit-media-workflow.md](docs/developer-cockpit-media-workflow.md): current contributor, owner, agent, media-package, and continuous-production contract.

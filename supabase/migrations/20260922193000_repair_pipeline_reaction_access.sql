...
-- The Contributor cold cut mechanically replaced "developer" but could not
-- translate the abbreviated access-mode value "dev". The application sends
-- the canonical "contributor" value, so Contributor and owner hearts were
-- rejected as invalid before they reached the reaction ledger.
...
comment on function public.cardforge_set_pipeline_heart(uuid, text, boolean, text, boolean, boolean) is
  'Atomically authorizes and changes one signed-in account reaction using canonical Contributor access modes.';

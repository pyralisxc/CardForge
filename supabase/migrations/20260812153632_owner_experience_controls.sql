...
alter table public.cardforge_owner_settings
  add column if not exists project_file_access text not null default 'creator_pass'
    check (project_file_access in ('free', 'creator_pass')),
  add column if not exists analytics_consent_presentation text not null default 'required_popup'
    check (analytics_consent_presentation in ('required_popup', 'popup', 'banner'));

comment on column public.cardforge_owner_settings.project_file_access is
  'CardForge-owned policy for portable project-file access. Finished export entitlement remains separate.';

comment on column public.cardforge_owner_settings.analytics_consent_presentation is
  'Public consent UI presentation only. Every mode preserves accept, session-only accept, and decline choices.';
...
with current_identity as (
  ...
  values (
    'privacy',
    'Privacy Policy',
    $privacy_consent_controls$CardForge Studio is a software product created and operated by Cameron Locke, an independent sole proprietor based in Oregon.
...
Policy changes may be made as CardForge and its data practices develop. An updated publication will identify its version and effective date, so review the current policy when you use the service.$privacy_consent_controls$
  )
), versioned as (
  select
    publication.slug,

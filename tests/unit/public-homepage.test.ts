...
describe('proof-led homepage', () => {
  ...
  it('places an interactive real-renderer walkthrough directly below the cover', () => {
    ...
    expect(showcase).toContain('ResponsiveSiteMediaImage');
    expect(showcase).not.toContain('unoptimized');
    expect(showcase).toContain('sizes="(min-width: 1280px) 1119px');
    expect(showcase).toContain('getSiteMediaFrameAspectRatio');
    expect(showcase).toContain("frameAspectRatio ? 'overflow-hidden' : 'max-h-[46rem] overflow-y-auto'");
    expect(showcase).toContain('h-auto w-full');

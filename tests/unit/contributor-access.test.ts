...
import {
  ...
  OWNER_CONTRIBUTOR_ACCESS_PROJECTION,
...
describe('Contributor access', () => {
  ...
  it('projects owner revision controls from the current trusted account session', () => {
    expect(resolveContributorAccessProjectionForSession({
      eligible: true,
      isOwner: true,
      sessionKey: 'owner-session',
      state: EMPTY_CONTRIBUTOR_ACCESS_SESSION_STATE,
    })).toMatchObject({
      active: true,
  ...
  it('clears an owner projection before the same account can return without owner access', () => {
    const priorOwnerState = {
      sessionKey: 'owner-session',
      projection: OWNER_CONTRIBUTOR_ACCESS_PROJECTION,
    };

    expect(shouldClearStoredContributorAccess({
      eligible: true,
      isOwner: true,
      sessionKey: 'owner-session',
    ...
    expect(resolveContributorAccessProjectionForSession({
      ...
      sessionKey: null,
      state: EMPTY_CONTRIBUTOR_ACCESS_SESSION_STATE,
    })).toBe(EMPTY_CONTRIBUTOR_ACCESS_SESSION_STATE.projection);
    expect(resolveContributorAccessProjectionForSession({
      eligible: true,
      isOwner: false,
      sessionKey: 'owner-session',

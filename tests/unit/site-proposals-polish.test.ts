...
describe('contributor surface polish contract', () => {
  ...
  it('keeps canonical campaign and media responsibilities in focused owners', () => {
    ...
    const composer = readFileSync(
      resolve(contentRoot, 'components', 'CampaignComposer.tsx'),
      'utf8',
    );
  ...
  it('makes package readiness visible without making optional package sections persistence requirements', () => {
    ...
    expect(getCampaignPackageReadiness({
      ...
      variants: [{
        service: 'facebook',
        text: 'The Contributor workspace is ready.',
        attachments: [{
          id: 'attachment-1',
          ...
          captionOverride: '',
          cropIntent: {},
          altText: 'The CardForge Contributor campaign queue.',
  ...
  it('shows reviewers campaign proof instead of approving an image count', () => {
    const details = readFileSync(
      sourcePath('features', 'marketing-content', 'components', 'CampaignPackageDetails.tsx'),
      'utf8',
    );
  ...
  it('gives every candidate a provider preview and a focused review editor', () => {
    ...
    const panel = readFileSync(
      sourcePath('features', 'marketing-content', 'components', 'CampaignWorkspace.tsx'),
      'utf8',
    );
    ...
    expect(queue).toContain('>Edit</Button>');
    expect(queue).toContain('CampaignPackageDetails');
    expect(panel).toContain('Back to campaigns');
  ...
  it('confirms live site publication and supports proposal withdrawal', () => {
    const proposalPanel = readFileSync(
      sourcePath('features', 'site-proposals', 'components', 'SiteProposalPanel.tsx'),
      'utf8',
    );

    expect(proposalPanel).toContain('WorkflowConfirmationDialog');
  ...
  it('gives owners reversible campaign-media retirement and guarded permanent deletion', () => {
    const mediaLibrary = readFileSync(
      sourcePath('features', 'marketing-content', 'components', 'CampaignMediaLibrary.tsx'),
      'utf8',
    );

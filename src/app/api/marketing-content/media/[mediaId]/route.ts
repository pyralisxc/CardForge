...
export async function GET(
  ...
  try {
    ...
    if (derivativeId && !derivative) return createApiErrorResponse(404, 'marketing_content_invalid', 'Campaign media derivative not found.');
    ...
    if (!supabase) return createApiErrorResponse(503, 'marketing_content_unavailable', 'Campaign media storage is not configured.');
    const { data, error } = await supabase.storage.from(bucket).download(path);
    if (error || !data) return createApiErrorResponse(404, 'marketing_content_invalid', 'Campaign media not found.');
    ...
    return createMarketingContentErrorResponse(error, 'Unable to load campaign media.');
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    if (typeof body.archived !== 'boolean') return createApiErrorResponse(400, 'marketing_content_invalid', 'Choose whether to retire or restore this media item.');
    ...
    return createMarketingContentErrorResponse(error, 'Unable to update campaign media retention.');
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (typeof body.confirmationFilename !== 'string' || !body.confirmationFilename.trim()) return createApiErrorResponse(400, 'marketing_content_invalid', 'Type the exact filename to confirm permanent deletion.');
    ...
    return createMarketingContentErrorResponse(error, 'Unable to permanently delete campaign media.');
  }
}

...
export async function GET(
  ...
  try {
    ...
    const derivative = derivativeId
      ? record.derivatives.find((item) => item.id === derivativeId)
      : null;
    if (derivativeId && !derivative) {
      return createApiErrorResponse(
        404,
        'developer_cockpit_request_invalid',
        'Campaign media derivative not found.',
      );
    }
    ...
    if (!supabase) {
      return createApiErrorResponse(
        ...
        'Campaign media storage is not configured.',
      );
    }
    ...
    return new Response(data, {
      status: 200,
      headers: {
        'Cache-Control': derivative?.exposure === 'public'

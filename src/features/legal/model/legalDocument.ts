...
Signed-in users with connected-storage access may connect Google Drive to store and work with .cardforge project files. CardForge asks Google for basic identity information and the drive.file permission. This limits CardForge to files and folders CardForge creates or that the user explicitly selects or authorizes for CardForge; CardForge does not request broad access to all files in the user's Drive. For those authorized locations, CardForge may read file and folder metadata, effective capabilities, thumbnails, and project contents as needed to discover, preview, open, refresh, save, recover, or move an explicitly removed CardForge project file to Google Drive Trash. Google Drive permissions remain authoritative for personal folders, shared folders, and Shared Drives.

CardForge stores the connected Google account identifier, selected Drive file and folder references, granted scopes, connection status, and an encrypted Google OAuth refresh credential in Supabase so the user can reconnect to authorized Drive project locations and compatible authenticated assistant workflows can operate on selected projects. Project files remain in Google Drive and use the user's Google storage. Authorized project bytes may pass through CardForge's server when opening a project or when an assistant workflow checks out or commits a selected project; browser saves may upload project bytes directly to Google's resumable upload endpoint. Disconnecting Google Drive asks Google to revoke CardForge's credential and removes CardForge's connection record; it does not delete the user's Drive project files. Revoking CardForge from the user's Google Account prevents future access until the user authorizes it again. CardForge uses Google Drive data only to provide, secure, and troubleshoot these user-directed storage and collaboration features, not for advertising.

CardForge's use and transfer to any other app of information received from Google APIs will adhere to the Google API Services User Data Policy, including the Limited Use requirements.
...
Browser IndexedDB data remains until you clear it or the browser removes it, and downloaded project files remain until you delete them from the places where you saved them. Private assistant working documents use an inactivity window tied to the current account plan: 12 hours for Free, 24 hours for Creator Pass, and 48 hours for Designer Pass, Contributor, and owner accounts by default. Opening or updating a document restarts its window; merely listing documents on the Account page does not. The CardForge owner may adjust these plan windows in the protected Profile utility. An expired or manually deleted assistant document remains in recoverable trash for 24 hours, then CardForge permanently removes its stored document and private artwork. Aggregate MCP usage may remain for operational, security, abuse-prevention, legal, or record-integrity needs. Other platform and provider records are retained for periods that vary by record, operational need, legal obligation, and provider setting. Some billing, legal, voting, attribution, published-asset, aggregate usage, and security records may need to remain after an account is disabled or deleted to preserve accurate platform history and system integrity.
...
Your agreement for the service is with ${DEFAULT_BUSINESS_IDENTITY.legalOperatorName} as the legal operator of ${DEFAULT_BUSINESS_IDENTITY.brandName}. CardForge lets users create Templates, generate previews, manage local projects, use connected assistant tools that create private cloud working documents, submit work to the Contributor Pipeline, and export content according to their account access. You are responsible for the content, artwork, data, trademarks, and intellectual property you bring into the tool or send through a connected assistant.
...
You keep ownership of the content you create. By using CardForge, you grant CardForge the limited permission needed to operate the service, render previews, process exports, preserve local/project state, and, when you submit assets to the Contributor Pipeline, review, display, publish, archive, and maintain those submitted assets as part of the shared Library.

The product is in active beta. Features, pricing, access levels, export behavior, Contributor rules, and Library availability may change as the service develops. Do not use CardForge for unlawful content, infringing content, malicious uploads, harassment, or activity that harms the platform or other users.
...
${DEFAULT_BUSINESS_IDENTITY.legalOperatorName} handles support for ${DEFAULT_BUSINESS_IDENTITY.brandName} as its owner and legal operator. For support, Contributor account requests, legal questions, billing questions, account problems, or asset Pipeline concerns, contact the support email listed on this site.

For fastest help, include the account email, the page or workflow where the issue happened, what you expected, what actually happened, and whether the issue involves a local project, export, Template, Pipeline asset, or billing/access state.
...
const contributorTermsBody = `${operatorDescription}

Your Contributor agreement is with ${DEFAULT_BUSINESS_IDENTITY.legalOperatorName} as the legal operator of ${DEFAULT_BUSINESS_IDENTITY.brandName}. Forge Review is the contribution path for CardForge. Contributors may submit Templates, icons, dividers, textures, frames, source files, element recipes, and other approved creative assets into the shared review Pipeline.
...
These Contributor Terms describe the current contribution model and do not create employment, partnership, guaranteed payment, or ownership of CardForge unless a separate written agreement says so.`;
...
export const DEFAULT_LEGAL_DOCUMENTS: LegalDocument[] = [
  createDefaultDocument('privacy', 'Privacy Policy', privacyBody, '2026-09-09'),

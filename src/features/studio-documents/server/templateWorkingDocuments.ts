...
import {
  requireAccountToolCapability,
  type AccountToolAccess,
} from '@/features/account/server';
import { requireContributionScope } from '@/features/contributor-access/server';
...
export const createTemplateWorkingDocument = async (
...
export const listTemplateWorkingDocuments = async (access: AccountToolAccess) => {
  requireAccountToolCapability(access, 'studio.ai.create');
  return listStudioDocumentsPage(access.user.id, await getStudioDocumentRetentionHours(access.entitlement));
};

export const getTemplateWorkingDocument = async (
  access: AccountToolAccess,
  documentId: string,
) => {
  requireAccountToolCapability(access, 'studio.ai.create');
...
export const updateTemplateWorkingDocument = async ({
  access,
  documentId,
  expectedRevision,
  input,
...
export const attachTemplateWorkingDocumentAsset = async ({
  access,
  documentId,
  expectedRevision,
  assetRequirementId,
  ...
  for (const asset of downloads) {
    ...
    try {
      ...
      throw new StudioDocumentStoreError(
        ...
        error instanceof PipelineRegistryCommandError
...
export const submitTemplateWorkingDocumentToPipeline = async ({
  access,
  documentId,
  templateId,
}: {

...
export const getActionsForRecord = (recipe: RecipeId, record: DetailRecord | null, _activeZone: ZoneId): readonly ActionDescriptor[] => {
  ...
  if (recipe === 'queue') {
    ...
    if (record.kind === 'forge-review') return [
      action({ id: 'owner.publish-contribution', label: 'Approve & publish', ownerFeature: 'pipeline', supportedObjectKinds: ['forge-review'], requiredPermission: 'owner', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'publication', automation: { kind: 'planned-mcp', capability: 'publish a reviewed contribution' }, result: 'mutation' }),
      action({ id: 'owner.request-changes', label: 'Request changes', ownerFeature: 'pipeline', supportedObjectKinds: ['forge-review'], requiredPermission: 'owner', scope: 'object', hierarchy: 'supporting', availability: available, commitment: 'none', automation: { kind: 'planned-mcp', capability: 'request contribution changes' }, result: 'mutation' }),
    ];
    if (record.kind === 'provider-attention') return [action({ id: 'contributor.review-provider', label: 'Review configuration', ownerFeature: 'site-proposals', supportedObjectKinds: ['provider-attention'], requiredPermission: 'contributor', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'permission', automation: human('provider'), result: 'provider-handoff' })];
    if (record.kind === 'operations-monitor') return [action({ id: 'contributor.inspect-monitor', label: 'Inspect monitor', ownerFeature: 'billing', supportedObjectKinds: ['operations-monitor'], requiredPermission: 'contributor', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'none', automation: human(), result: 'preview' })];

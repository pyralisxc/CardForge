...
export const homeSnapshotItems: readonly SettingItem[] = ([
  { id: 'home-plan', kind: 'account-plan', eyebrow: 'Plan', title: 'Plan', summary: 'Product access and output policy', value: 'Free · 1 slot', status: 'Active', tone: 'success', icon: CreditCard, meta: [['Plan', 'Free'], ['Cloud slots', '1 available'], ['Clean export', 'Upgrade required'], ['Management', 'Compare plans']] },
...
export const getActionsForRecord = (recipe: RecipeId, record: DetailRecord | null, activeZone: ZoneId): readonly ActionDescriptor[] => {
  if (recipe === 'home') {
    if (!record || record.kind === 'set') return [action({ id: 'home.resume-set', label: 'Resume in Studio', ownerFeature: 'card-generator', supportedObjectKinds: ['set'], supportedSources: ['browser-local', 'google-drive', 'local-folder', 'cardforge-cloud'], requiredPermission: 'creator', scope: record ? 'object' : 'zone', hierarchy: 'primary', availability: available, commitment: 'none', automation: human(), result: 'navigation' })];
    if (record.kind === 'account-plan') return [action({ id: 'account.compare-plans', label: 'Compare plans', ownerFeature: 'billing', supportedObjectKinds: ['account-plan'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'financial', automation: human('provider'), result: 'provider-handoff' })];
    if (record.kind === 'storage-summary') return [action({ id: 'home.open-library', label: 'Open Library', ownerFeature: 'storage-management', supportedObjectKinds: ['storage-summary'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'none', automation: human(), result: 'navigation' })];
    if (record.kind === 'connection-summary') return [action({ id: 'account.manage-connections', label: 'Manage connections', ownerFeature: 'storage-management', supportedObjectKinds: ['connection-summary'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'permission', automation: human('provider'), result: 'provider-handoff' })];
    return [action({ id: 'account.open-security', label: 'Open security controls', ownerFeature: 'account', supportedObjectKinds: ['security-summary'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'permission', automation: human('provider'), result: 'provider-handoff' })];
  }

  if (recipe === 'collection') {
    if (!record) return [action({ id: 'library.add-source', label: 'Add to Library', ownerFeature: 'storage-management', supportedObjectKinds: [], requiredPermission: 'creator', scope: 'zone', hierarchy: 'primary', availability: available, commitment: 'permission', automation: human('provider'), result: 'provider-handoff' })];
    if (record.kind === 'set') return [
      action({ id: 'library.open-set', label: 'Open in Studio', ownerFeature: 'card-generator', supportedObjectKinds: ['set'], supportedSources: ['browser-local', 'google-drive', 'local-folder'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'none', automation: human(), result: 'navigation' }),
      action({ id: 'library.export-set', label: 'Export package', ownerFeature: 'card-generator', supportedObjectKinds: ['set'], supportedSources: ['browser-local', 'google-drive', 'local-folder'], requiredPermission: 'creator', scope: 'object', hierarchy: 'supporting', availability: available, commitment: 'none', automation: human(), result: 'download' }),
    ];
    if (record.kind === 'project') return [action({ id: 'library.open-project', label: 'Open project', ownerFeature: 'project', supportedObjectKinds: ['project'], supportedSources: ['google-drive'], revisionPolicy: 'current-required', requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'permission', automation: { kind: 'published-mcp', tools: ['list_connected_projects', 'checkout_project'] }, result: 'navigation' })];
    if (record.kind === 'temporary-draft') return [action({ id: 'library.save-draft', label: 'Make durable', ownerFeature: 'storage-management', supportedObjectKinds: ['temporary-draft'], supportedSources: ['temporary'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'permission', automation: human(), result: 'mutation' })];
    return [action({ id: 'library.inspect-template', label: 'Inspect template', ownerFeature: 'template-editor', supportedObjectKinds: ['template'], supportedSources: ['browser-local'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'none', automation: human(), result: 'preview' })];
  }

  if (recipe === 'profile') return record?.kind === 'preference' ? [
    action({ id: 'profile.save-preference', label: 'Save preference', ownerFeature: 'account', supportedObjectKinds: ['preference'], supportedSources: ['browser-local'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'none', automation: human(), result: 'mutation' }),
  ] : [action({ id: 'profile.open-provider', label: 'Open provider controls', ownerFeature: 'account', supportedObjectKinds: ['provider-setting'], requiredPermission: 'creator', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'permission', automation: human('provider'), result: 'provider-handoff' })];
  ...
  if (recipe === 'queue') {
    ...
    if (record.kind === 'provider-attention') return [action({ id: 'developer.review-provider', label: 'Review configuration', ownerFeature: 'developer-cockpit', supportedObjectKinds: ['provider-attention'], requiredPermission: 'developer', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'permission', automation: human('provider'), result: 'provider-handoff' })];
    if (record.kind === 'operations-monitor') return [action({ id: 'developer.inspect-monitor', label: 'Inspect monitor', ownerFeature: 'billing', supportedObjectKinds: ['operations-monitor'], requiredPermission: 'developer', scope: 'object', hierarchy: 'primary', availability: available, commitment: 'none', automation: human(), result: 'preview' })];

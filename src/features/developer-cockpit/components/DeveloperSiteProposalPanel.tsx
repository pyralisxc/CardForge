...
export function DeveloperSiteProposalPanel({ cockpit, onChange }: { cockpit: DeveloperSiteWorkspaceView; onChange: (cockpit: DeveloperSiteWorkspaceView) => void }) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border border-[#5f4526] bg-[#15100a] p-4"><div className="flex items-center gap-3"><PencilLine className="h-5 w-5 text-[#e2aa4a]" /><div><p className="text-xs uppercase tracking-[0.16em] text-[#e2aa4a]">Public site copy</p><p className="text-sm text-[#c7b288]">Propose and review changes to owner-managed public text.</p></div></div>{canPropose && !showComposer ? <Button type="button" className="min-h-11" onClick={() => { setDraft(emptyDraft(cockpit)); setEditing(null); setShowComposer(true); }}><Plus className="mr-2 h-4 w-4" />New site proposal</Button> : null}</div>
      ...
      {showComposer && canPropose ? <article className="border border-[#5f4526] bg-[#15100a] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.16em] text-[#e2aa4a]">{editing ? 'Editing proposal' : 'New proposal'}</p><h2 className="font-serif text-2xl text-[#fff1c7]">Improve public site copy</h2></div><Button type="button" className="min-h-11" variant="outline" onClick={closeComposer}>Close editor</Button></div><div className="mt-4 grid gap-3"><label className="grid gap-1 text-xs text-[#c7b288]">Site text to update<select className={fieldClassName} value={draft.slug} disabled={Boolean(editing)} onChange={(event) => selectBlock(event.target.value as SiteContentBlockSlug)}>{cockpit.siteContentBlocks.map((block) => <option key={block.slug} value={block.slug}>{block.group} / {block.section} / {block.label}</option>)}</select></label><label className="grid gap-1 text-xs text-[#c7b288]"><span className="flex justify-between"><span>Proposed copy</span><span>{draft.proposedBody.length}/{selectedBlock?.maxLength ?? 800}</span></span><textarea className={`${fieldClassName} min-h-32`} maxLength={selectedBlock?.maxLength ?? 800} value={draft.proposedBody} onChange={(event) => setDraft({ ...draft, proposedBody: event.target.value })} /></label><label className="grid gap-1 text-xs text-[#c7b288]"><span className="flex justify-between"><span>Why this helps</span><span>{draft.rationale.length}/800</span></span><textarea className={`${fieldClassName} min-h-24`} maxLength={800} value={draft.rationale} onChange={(event) => setDraft({ ...draft, rationale: event.target.value })} placeholder="Name the ambiguity, audience need, SEO intent, or product truth this improves." /></label></div><Button type="button" className="mt-4 min-h-11" disabled={Boolean(busy)} onClick={() => void save()}>{busy === 'create' || busy?.startsWith('save:') ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileDiff className="mr-2 h-4 w-4" />}{editing ? 'Save changes' : 'Create proposal'}</Button></article> : null}
      ...
      <div className="space-y-3">
        {cockpit.siteProposals.length === 0 ? <article className="border border-dashed border-[#5f4526] bg-[#100c08] p-6 text-center"><FileDiff className="mx-auto h-6 w-6 text-[#a98a55]" /><p className="mt-3 text-sm text-[#c7b288]">No site-copy proposals yet.</p></article> : cockpit.siteProposals.map((proposal) => {

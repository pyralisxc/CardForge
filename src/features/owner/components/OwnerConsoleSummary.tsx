...
export function OwnerConsoleSummary({ payload, lastOwnerSaveAt }: { payload: OwnerConsoleResponse; lastOwnerSaveAt: string | null }) {
  ...
  return (
    <section className="border border-[#6d4f2b] bg-[#15100a] p-4">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{status.map(([label, value, ready]) => <div key={label} className="flex min-h-14 items-center justify-between gap-3 border border-[#3c2c1b] bg-[#100c08] px-3 py-2"><div><span className="block text-[10px] uppercase tracking-[0.14em] text-[#8f7b57]">{label}</span><span className="block text-sm font-semibold text-[#ffe7ad]">{value}</span></div><span className={`h-2.5 w-2.5 ${ready ? 'bg-[#8fca72]' : 'bg-[#e2aa4a]'}`} /></div>)}</div>

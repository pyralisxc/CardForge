...
} from '@/features/site-proposals/client/api';
...
const emptyDraft = (workspace: SiteProposalWorkspace): ProposalDraft => ({
  slug: workspace.siteContentBlocks[0]?.slug ?? 'landing.hero.headline',
  proposedBody: workspace.siteContentBlocks[0]?.body ?? '',
  rationale: '',
});

export function SiteProposalPanel({ workspace, onChange }: { workspace: SiteProposalWorkspace; onChange: (workspace: SiteProposalWorkspace) => void }) {
  const canPropose = workspace.scopes.includes('site.propose');
  const canPublish = workspace.scopes.includes('site.publish');
  const [showComposer, setShowComposer] = useState(false);
  const [draft, setDraft] = useState<ProposalDraft>(() => emptyDraft(workspace));
  const [editing, setEditing] = useState<SiteContentProposal | null>(null);
  const [reviewNotes, setReviewNotes] = useState<Record<string, string>>({});
  const [busy, setBusy] = useState<string | null>(null);
  const [error, setError] = useState('');
  const [message, setMessage] = useState('');
  const selectedBlock = workspace.siteContentBlocks.find((block) => block.slug === draft.slug);

  const run = async (key: string, success: string, action: () => Promise<SiteProposalWorkspace>) => {
  ...
  const closeComposer = () => { setEditing(null); setDraft(emptyDraft(workspace)); setShowComposer(false); };
  const selectBlock = (slug: SiteContentBlockSlug) => { const block = workspace.siteContentBlocks.find((candidate) => candidate.slug === slug); setDraft((current) => ({ ...current, slug, proposedBody: block?.body ?? '' })); };
  const save = async () => {
    const saved = editing
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-[var(--cf-border)] py-3"><div className="flex items-center gap-3"><PencilLine className="h-5 w-5 text-[var(--cf-accent-strong)]" /><div><p className="text-xs uppercase tracking-[0.16em] text-[var(--cf-accent-strong)]">Public site copy</p><p className="text-sm text-[var(--cf-text-muted)]">{workspace.siteProposals.length} proposal{workspace.siteProposals.length === 1 ? '' : 's'} · owner-published</p></div></div>{canPropose && !showComposer ? <Button type="button" className="min-h-11" onClick={() => { setDraft(emptyDraft(workspace)); setEditing(null); setShowComposer(true); }}><Plus className="mr-2 h-4 w-4" />New site proposal</Button> : null}</div>

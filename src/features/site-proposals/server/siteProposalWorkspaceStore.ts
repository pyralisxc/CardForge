...
import { listContributorAccessProfiles } from '@/features/contributor-access/server';
...
export const getSiteProposalWorkspace = async (
  access: ContributorAccess,
  ...
  const [proposalResult, siteContentBlocks, profiles] = await Promise.all([
    ...
    listContributorAccessProfiles(access.isOwner),
  ]);
  return {

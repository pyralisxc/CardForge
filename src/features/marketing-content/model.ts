...
export interface MarketingChannelVariant {
...
export interface MarketingContentPackage {
  id: string;
...
export const normalizeCampaignInput = (input: CampaignInput): CampaignInputResult => {
  ...
  const seenServices = new Set<MarketingChannel>();
  const variants: Array<{ service: MarketingChannel; text: string; attachments: CampaignAttachmentInput[] }> = [];
  for (const value of input.variants) {
    const candidate = objectValue(value);
    if (!candidate || typeof candidate.service !== 'string' || !MARKETING_CHANNELS.includes(candidate.service as MarketingChannel)) return { ok: false, message: 'Choose a supported marketing channel.' };
    ...
    const service = candidate.service as MarketingChannel;
    ...
    if (!text) return { ok: false, message: `${MARKETING_CHANNEL_LABELS[service]} copy is required.` };
    const textLimit = service === 'instagram' ? 2_200 : CAMPAIGN_FIELD_LIMITS.variantText;
    if (text.length > textLimit) return { ok: false, message: `${MARKETING_CHANNEL_LABELS[service]} copy must be ${textLimit.toLocaleString('en-US')} characters or fewer.` };
...
const contributorCampaignTransitions: Partial<Record<MarketingContentStatus, MarketingContentStatus[]>> = { draft: ['submitted', 'cancelled'], changes_requested: ['draft', 'submitted', 'cancelled'], submitted: ['cancelled'] };
const ownerCampaignTransitions: Partial<Record<MarketingContentStatus, MarketingContentStatus[]>> = { draft: ['submitted', 'cancelled'], submitted: ['changes_requested', 'approved', 'cancelled'], changes_requested: ['cancelled'], approved: ['provider_draft', 'scheduled', 'failed', 'cancelled'], provider_draft: ['scheduled', 'published', 'failed', 'cancelled'], scheduled: ['published', 'failed', 'cancelled'], failed: ['approved', 'cancelled'] };
export const canTransitionCampaign = (from: MarketingContentStatus, to: MarketingContentStatus, actor: 'contributor' | 'owner'): boolean => (actor === 'owner' ? ownerCampaignTransitions : contributorCampaignTransitions)[from]?.includes(to) ?? false;

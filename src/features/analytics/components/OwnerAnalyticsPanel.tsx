...
export function OwnerAnalyticsPanel({ publicAppUrl }: { publicAppUrl: string }) {
  ...
  return (
    <div className="space-y-4">
      <section className="border border-[#6d4f2b] bg-[#15100a] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><BarChart3 className="h-6 w-6 text-[#d8b365]" /><div><p className="text-xs uppercase tracking-[0.18em] text-[#a98a55]">Owner-only measurement</p><h2 className="font-serif text-2xl text-[#fff1c7]">Organic Analytics Cockpit</h2><p className="mt-1 text-sm text-[#c7b288]">Google owns measurements. CardForge presents anonymous campaign and creator-journey signals.</p></div></div><div className="text-right"><Button type="button" variant="outline" onClick={() => void refresh()} disabled={isLoading}><RefreshCw className={`mr-2 h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />Refresh</Button><p className="mt-2 text-xs text-[#8f7b57]">Updated {formatDate(snapshot.capturedAt)} · auto-refreshes every minute</p></div></div>{error ? <p className="mt-3 border border-[#7d3d32] bg-[#1b0d09] p-3 text-sm text-[#ffd0c6]">{error}</p> : null}{snapshot.warnings.map((warning) => <p key={warning} className="mt-3 border border-[#8c6436] bg-[#1b1209] p-3 text-sm text-[#f0bd75]">{warning}</p>)}</section>

  ...
  return (
    <section className="border border-[#6d4f2b] bg-[#15100a] p-4">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        <label className="text-xs uppercase tracking-[0.12em] text-[#a98a55]">Post label<input className={`${inputClassName} mt-2 normal-case`} value={content} onChange={(event) => setContent(event.target.value)} placeholder="group_post_01" /></label>
      </div>
...
function SearchTable({ title, rows, available }: { title: string; rows: OwnerAnalyticsSnapshot['search']['queries']; available: boolean }) {
  return <section className="overflow-x-auto border border-[#4a3823] bg-[#100c08] p-4"><h3 className="font-serif text-lg text-[#fff1c7]">{title}</h3>{!available ? <p className="mt-3 text-sm text-[#f0bd75]">This Search Console report is temporarily unavailable.</p> : rows.length === 0 ? <p className="mt-3 text-sm text-[#8f7b57]">Search Console has no data for this date range yet.</p> : <table className="mt-4 min-w-full text-left text-sm"><thead className="text-xs uppercase tracking-[0.12em] text-[#8f7b57]"><tr><th className="pb-2 pr-4">Result</th><th className="pb-2 pr-4">Clicks</th><th className="pb-2 pr-4">Impressions</th><th className="pb-2">Position</th></tr></thead><tbody>{rows.map((row) => <tr key={row.label} className="border-t border-[#352716] text-[#d9c6a1]"><td className="max-w-xs truncate py-3 pr-4" title={row.label}>{row.label}</td><td className="py-3 pr-4">{formatNumber(row.clicks)}</td><td className="py-3 pr-4">{formatNumber(row.impressions)}</td><td className="py-3">{formatPosition(row.position)}</td></tr>)}</tbody></table>}</section>;
}

...
interface HomeProjectWorkspaceOptions {
...
export function useHomeProjectWorkspace(options: HomeProjectWorkspaceOptions) {
  ...
  return {
    actions: {
      ...
      openEditDialog, removeGeneratedCards, renameCardSet, reorderGeneratedCard, setActiveCardSetId, setCardPositions,
      setCardsTag, setGeneratorSelectedBackingTemplateId, setGeneratorSelectedTemplateId, setStudioView,
      setTemplateEditorSelectedTemplateId, updateCardSetOrganization,
    },
    state: {
      ...
      availableFields, cardSets, displayCards, effectiveMoveTargetId: otherSets.some((set) => set.id === options.moveTargetId) ? options.moveTargetId : otherSets[0]?.id ?? '',

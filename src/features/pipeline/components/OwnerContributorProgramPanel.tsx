...
import {
  ...
  getPipelineVotingPresetLabel,
  type PipelineProgramSettings,
  type PipelineVotingPreset,
  type PipelineStatus,
  type PipelineType,
...
export function OwnerContributorProgramPanel({
  initialStatusFilter = 'all',
}: {
  initialStatusFilter?: PipelineStatus | 'all';
}) {
  const { toast } = useToast();
  const [program, setProgram] = useState<PipelineProgramView | null>(null);
  const [settings, setSettings] = useState<PipelineProgramSettings | null>(null);
  ...
  const [libraryAssetType, setLibraryAssetType] = useState<PipelineType | 'all'>('all');
  const [libraryStatus, setLibraryStatus] = useState<PipelineStatus | 'all'>(initialStatusFilter);
  ...
  const loadProgram = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await readApiErrorMessage(response, 'Unable to load the Contributor program.'));
      const body = await response.json() as PipelineItemsResponse;
      setProgram(body.program);
      setSettings(body.program.settings);
      hasLoadedRef.current = true;
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unable to load the Contributor program.';
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await readApiErrorMessage(response, 'Unable to save the Contributor program.'));
      const body = await response.json() as PipelineItemsResponse;
      setSettings(body.program.settings);
      await loadProgram();
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Unable to save the Contributor program.',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  const applyVotingPreset = (preset: PipelineVotingPreset) => {
    if (!settings) return;
    setSettings(buildPipelineVotingPresetSettings(settings, preset, program?.activeContributorCount ?? 1));
  };

  if (isLoading) {
    return (
      <section className="border border-[var(--cf-border)] bg-[var(--cf-surface)] p-6 text-[var(--cf-text-muted)]">
        Loading Contributor program...
      </section>
    );
  }
    ...
    return (
      <section className="border border-[var(--cf-danger-border)] bg-[var(--cf-danger-surface-muted)] p-6 text-[var(--cf-danger)]">
        <div className="flex items-center gap-3 text-[#ffb8a8]">
          ...
          <h2 className="font-serif text-2xl text-[#ffe1d8]">Contributor program unavailable</h2>
        </div>
        <p className="mt-3 text-sm leading-6">
          {loadError ?? 'Unable to load the Contributor program.'}
        </p>
        <p className="mt-3 text-sm leading-6 text-[#e7b3a8]">
          Confirm the Contributor Pipeline Supabase migration has been applied, then refresh this page.
        </p>
        <Button
  ...
  return (
    ...
    <section className="border border-[var(--cf-warning-border)] bg-[var(--cf-surface)] p-6">
      <OwnerContributorProgramOverview program={program} settings={settings} lastSavedLabel={lastSavedLabel} />
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">
        <div className="border border-[var(--cf-border)] bg-[var(--cf-surface-inset)] p-4">
          <div className="flex items-center justify-between gap-3">
            <h3 className="font-serif text-xl text-[var(--cf-text-strong)]">Monthly Contributor contract</h3>
            <FieldHelp text="These controls define who can participate, how often Contributors can upload, and the monthly contribution expectation." />
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <NumberField label="Max Contributors" help="Total active Contributor slots in the curated program." value={settings.maxActiveContributors} onChange={(value) => setSettings({ ...settings, maxActiveContributors: value })} />
            <NumberField label="Submission allowance" help="Maximum site-submitted assets one Contributor can upload each calendar month. Submissions left is calculated from this." value={settings.monthlySubmissionLimit} onChange={(value) => setSettings({ ...settings, monthlySubmissionLimit: value })} />
            ...
            <NumberField label="Required published" help="Minimum published assets expected from each active Contributor per calendar month." value={settings.monthlyPublishedRequirement} onChange={(value) => setSettings({ ...settings, monthlyPublishedRequirement: value })} />
          </div>
        </div>
        ...
        <div className="border border-[var(--cf-border)] bg-[var(--cf-surface-inset)] p-4">
          ...
          <div className="mt-4 border border-[#342719] bg-[var(--cf-surface)] p-3">
            ...
            <p className="mt-2 text-xs leading-5 text-[var(--cf-text-muted)]">
              Start with solo review while you are the only Contributor, then raise vote gates as the roster grows.
            </p>
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {(['solo', 'currentRoster', 'launchRoster', 'fullCouncil'] as PipelineVotingPreset[]).map((preset) => (
                <Button
                  key={preset}
                  type="button"
                  size="sm"
                ...
                >
                  {getPipelineVotingPresetLabel(preset, program.activeContributorCount)}
                </Button>
              ))}
            </div>
          </div>
            ...
            <tbody>
              {PIPELINE_TYPES.map((type) => (
                <tr key={type} className="border-b border-[#342719] last:border-b-0">
                  <td className="py-3 pr-3 text-[var(--cf-accent-text)]">{getPipelineTypeLabel(type)}</td>
                  <td className="px-3 py-3">
                    <CompactNumberField
                      ariaLabel={`${getPipelineTypeLabel(type)} starter cap`}
                  ...
                  <td className="px-3 py-3">
                    <CompactNumberField
                      ariaLabel={`${getPipelineTypeLabel(type)} creator pass cap`}
              ...
              <tbody>
                {program.assetTypeSummaries.map((summary) => (
                  <tr key={summary.assetType} className="border-b border-[#342719] last:border-b-0">
                    <td className="py-3 pr-3 text-[var(--cf-accent-text)]">{getPipelineTypeLabel(summary.assetType)}</td>
      ...
      <Button className="mt-5 bg-[var(--cf-accent-strong)] text-[var(--cf-accent-contrast)] hover:bg-[var(--cf-accent)]" disabled={isSaving} onClick={saveSettings}>
        ...
        {isSaving ? 'Saving Contributor program...' : 'Save Contributor program'}
      </Button>
        </TabsContent>
      </Tabs>

...
export function EditSubmissionForm({
  ...
  return (
    <div className="mt-3 grid gap-3 border border-[var(--cf-border)] bg-[var(--cf-surface-inset)] p-3">
      <label className="grid gap-1 text-xs uppercase tracking-[0.12em] text-[var(--cf-text-subtle)]">
        Name
        <input className="border border-[var(--cf-border)] bg-[var(--cf-canvas)] p-3 text-sm normal-case tracking-normal text-[var(--cf-accent-text)]" value={name} onChange={(event) => onNameChange(event.target.value)} />
      </label>
      <label className="grid gap-1 text-xs uppercase tracking-[0.12em] text-[var(--cf-text-subtle)]">
        Preview URL
        <input className="border border-[var(--cf-border)] bg-[var(--cf-canvas)] p-3 text-sm normal-case tracking-normal text-[var(--cf-accent-text)]" value={previewUrl} onChange={(event) => onPreviewUrlChange(event.target.value)} />
      </label>
      <label className="grid gap-1 text-xs uppercase tracking-[0.12em] text-[var(--cf-text-subtle)]">
        Description
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="outline" className="border-[var(--cf-border)] bg-transparent text-[var(--cf-accent-text)]" disabled={isSaving} onClick={onCancel}>
...
export function AssetRow({
  ...
  return (
    <div className="border border-[var(--cf-border-subtle)] bg-[var(--cf-canvas)] p-3">
      <div className="grid gap-3 sm:grid-cols-[4rem_1fr_auto] sm:items-center">
        <div className="grid h-16 w-16 place-items-center overflow-hidden border border-[var(--cf-border)] bg-[var(--cf-surface)] bg-[linear-gradient(45deg,rgba(255,255,255,0.04)_25%,transparent_25%,transparent_75%,rgba(255,255,255,0.04)_75%),linear-gradient(45deg,rgba(255,255,255,0.04)_25%,transparent_25%,transparent_75%,rgba(255,255,255,0.04)_75%)] bg-[length:16px_16px] bg-[position:0_0,8px_8px]">
          <AssetPreview submission={submission} templatePreviews={templatePreviews} />
        </div>
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <p className="font-medium text-[var(--cf-accent-text)]">{submission.name}</p>
            <span className="border border-[var(--cf-border)] px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[var(--cf-accent)]">
              Status: {getPipelineStatusLabel(submission.status)}
            </span>
            ...
            <span className={`border px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] ${tierClasses[submission.calculatedAccessTier]}`}>
              Tier: {getPipelineTierLabel(submission.calculatedAccessTier)}
            </span>
          </div>
          ...
          <div className="mt-2 grid gap-1">
            <div className="h-1.5 overflow-hidden bg-[#2b2116]" aria-hidden="true">
              <div className="h-full bg-[var(--cf-accent)]" style={{ width: `${progressPercent}%` }} />
            </div>
      ...
      {expanded ? (
        <div className="mt-3 grid gap-3 border-t border-[var(--cf-border-subtle)] pt-3 lg:grid-cols-[minmax(14rem,22rem)_1fr]">
          <div className="grid min-h-48 place-items-center overflow-hidden border border-[var(--cf-border)] bg-[var(--cf-surface)] bg-[linear-gradient(45deg,rgba(255,255,255,0.04)_25%,transparent_25%,transparent_75%,rgba(255,255,255,0.04)_75%),linear-gradient(45deg,rgba(255,255,255,0.04)_25%,transparent_25%,transparent_75%,rgba(255,255,255,0.04)_75%)] bg-[length:18px_18px] bg-[position:0_0,9px_9px]">
            <AssetPreview submission={submission} templatePreviews={templatePreviews} expanded />
          </div>
          <div className="text-sm leading-6 text-[var(--cf-text-muted)]">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <div className="border border-[var(--cf-border-subtle)] bg-[var(--cf-surface-inset)] p-3">
                <p className="text-xs uppercase tracking-[0.12em] text-[var(--cf-text-subtle)]">Status meaning</p>
                <p className="mt-1 text-xs leading-5 text-[var(--cf-text-muted)]">{getPipelineStatusDescription(submission.status)}</p>
              </div>
              <div className="border border-[var(--cf-border-subtle)] bg-[var(--cf-surface-inset)] p-3">
                <p className="text-xs uppercase tracking-[0.12em] text-[var(--cf-text-subtle)]">Tier meaning</p>
                <p className="mt-1 text-xs leading-5 text-[var(--cf-text-muted)]">{getPipelineTierDescription(submission.calculatedAccessTier)}</p>
              </div>
              <div className="border border-[var(--cf-border-subtle)] bg-[var(--cf-surface-inset)] p-3">
                <p className="text-xs uppercase tracking-[0.12em] text-[var(--cf-text-subtle)]">Next step</p>
                <p className="mt-1 text-xs leading-5 text-[var(--cf-text-muted)]">{getSubmissionNextStep(submission, program)}</p>
              </div>
            </div>
            <dl className="mt-3 grid gap-2 text-xs text-[var(--cf-text-subtle)] sm:grid-cols-2">
              <div><dt className="uppercase tracking-[0.12em]">Contributor</dt><dd className="break-all text-[var(--cf-text-muted)]">{getContributorLabel(submission)}</dd></div>
              <div><dt className="uppercase tracking-[0.12em]">Automatic result</dt><dd className="break-all text-[var(--cf-text-muted)]">{getPipelineStatusLabel(submission.automatedStatus)} / {getPipelineTierLabel(submission.automatedAccessTier)}</dd></div>
...
function AssetPreview({
  ...
  if (proposedTemplate || template) {
    ...
    return (
      <div className="max-h-[26rem] w-full overflow-auto p-4">
        <CardPreview
          card={{ template: primaryTemplate, data: primaryTemplate.templatePreviewData ?? {}, uniqueId: `pipeline-preview-${submission.id}` }}
          targetWidthPx={260}
          isEditorPreview
        />
      </div>
  ...
  if (submission.assetType === 'fonts') {
    const fontFamily = `pipeline-preview-${submission.id}`;
    ...
    return (
      <div className={`grid h-full w-full place-items-center text-center ${expanded ? 'gap-4 p-6' : 'p-2'}`}>
        ...
        <p
          className={expanded ? 'text-4xl leading-none text-[var(--cf-accent-text)]' : 'text-xl leading-none text-[var(--cf-accent-text)]'}
        ...
        {expanded ? (
          <div className="space-y-2">
            <p className="text-sm text-[var(--cf-accent-text)]" style={{ fontFamily: fontUrl ? `"${fontFamily}", serif` : undefined }}>
  ...
  return (
    <div className={`grid h-full w-full place-items-center text-center text-[var(--cf-text-muted)] ${expanded ? 'gap-2 p-6' : 'px-2'}`}>
      {expanded ? <Archive className="mx-auto h-8 w-8 text-[var(--cf-text-subtle)]" /> : null}
      <p className={`${expanded ? 'text-sm font-medium text-[var(--cf-accent-text)]' : 'text-[10px] uppercase tracking-[0.12em] text-[var(--cf-text-subtle)]'}`}>
        {getPipelineTypeLabel(submission.assetType, { plural: false })}
      </p>
      {expanded ? <p className="text-xs leading-5 text-[var(--cf-text-subtle)]">{message}</p> : null}
    </div>
  );
}
    ...
    >
      ...
      {expanded ? (
        <div className="text-center">
          <p className="font-serif text-lg text-[var(--cf-accent-text)]">{recipe.name}</p>
          <p className="mt-1 text-[10px] uppercase tracking-[0.14em] text-[var(--cf-text-subtle)]">
...
function TemplateComparisonPreview({
  ...
  return (
    <div className="grid content-start justify-items-center gap-2">
      ...
      <CardPreview
        card={{ template, data: template.templatePreviewData ?? {}, uniqueId: `pipeline-preview-${id}` }}
        targetWidthPx={240}
        isEditorPreview
      />
    </div>

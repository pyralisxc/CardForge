...
import { DecisionCard } from '@/features/pipeline/components/OwnerContributorProgramControls';
import type { PipelineProgramView } from '@/features/pipeline/lib/pipelineProgram';
import type { PipelineProgramSettings } from '@/features/pipeline/lib/pipelineItems';

export function OwnerContributorProgramOverview({
  program,
  settings,
  lastSavedLabel,
}: {
  ...
  return (
    <>
      ...
      <p className="mt-3 text-sm leading-6 text-[var(--cf-text-muted)]">
        Control Contributor slots, contribution rules, vote thresholds, library visibility, and per-type caps. Published assets are the only rows loaded into creator-facing Studio libraries; everything else remains visible in the Pipeline.
      </p>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-4">
        <DecisionCard label="Program status" body={program.configured ? 'Supabase contributor tables are connected and accepting submissions.' : 'Contributor tables are not configured yet.'} />
        <DecisionCard label="Submissions" body={`${program.totalSubmissionCount} total contributor asset submission${program.totalSubmissionCount === 1 ? '' : 's'} in the review system.`} />
        <DecisionCard label="Voteable assets" body={`${program.totalVoteableCount} asset${program.totalVoteableCount === 1 ? '' : 's'} can still receive contributor votes, including live assets whose automatic signal remains active.`} />
        <DecisionCard label="Active contributors" body={`${program.activeContributorCount} active contributor${program.activeContributorCount === 1 ? '' : 's'} currently count toward voting presets.`} />
        ...
        <DecisionCard label="Owner vote mode" body={settings.ownerVoteWeight === 1 ? 'Owner votes count like one Contributor vote.' : `Owner votes count as ${settings.ownerVoteWeight}x signal.`} />
      </div>

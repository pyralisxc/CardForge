...
} from './developerAssetRegistryCommands';
...
const requireSupabase = () => {
  ...
  if (!supabase) {
    throw new DeveloperAssetRegistryCommandError('The Forge Pipeline database is not configured.', 503);
  }
  return supabase;
};
...
export const getOwnerStudioRoutingPage = async ({
  ...
  if (registryResult.error || countsResult.error) {
    ...
    throw new DeveloperAssetRegistryCommandError('Unable to load the Studio Map.', 503);
  }
  ...
  const items = ((registryResult.data ?? []) as RegistryRoutingRow[]).flatMap((row) => {
    ...
    return [{
      ...
      developerSubmissionId: row.developer_submission_id,
...
export const updateOwnerStudioRouting = async (input: {
  ...
  if (!assetId || !mode || !Array.isArray(input.destinations) || !Number.isInteger(sortOrder) || sortOrder < 0 || sortOrder > 100000 || typeof input.featured !== 'boolean') {
    throw new DeveloperAssetRegistryCommandError('Studio placement requires a valid asset, mode, destinations, order, and featured state.', 400);
  }
  if (destinations.length !== input.destinations.length) {
    throw new DeveloperAssetRegistryCommandError('One or more Studio destinations are invalid.', 400);
  }

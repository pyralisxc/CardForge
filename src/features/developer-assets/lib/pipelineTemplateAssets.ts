...
import { DEVELOPER_ASSET_STORAGE_BUCKET } from './developerAssetUploadPolicy';
...
const requireSupabase = () => {
  ...
  if (!supabase) {
    throw new DeveloperAssetRegistryCommandError('Pipeline Template media storage is not configured yet.', 503);
  }
  return supabase;
};
...
const parseImageDataUri = (value: string): Buffer | null => {
  ...
  if (!['image/png', 'image/jpeg', 'image/webp', 'image/svg+xml'].includes(mimeType ?? '')) {
    throw new DeveloperAssetRegistryCommandError('Pipeline Template artwork must be PNG, JPEG, WebP, or SVG.', 400);
  }
  try {
    ...
    if (bytes.length <= 0 || bytes.length > MAX_SOURCE_BYTES) {
      throw new DeveloperAssetRegistryCommandError('Pipeline Template artwork must be 2.4 MB or smaller before normalization.', 413);
    }
    return bytes;
  } catch (error) {
    if (error instanceof DeveloperAssetRegistryCommandError) throw error;
    throw new DeveloperAssetRegistryCommandError('Pipeline Template artwork contains an invalid data URI.', 400);
  }
};
...
const normalizeImage = async (source: Buffer): Promise<Buffer> => {
  try {
    ...
    if (error instanceof DeveloperAssetRegistryCommandError) throw error;
    throw new DeveloperAssetRegistryCommandError('Pipeline Template artwork could not be decoded safely.', 400);
  }
};
...
const storeNormalizedAsset = async (bytes: Buffer): Promise<string> => {
  ...
  if (existing.error && existing.data !== false) {
    throw new DeveloperAssetRegistryCommandError('Unable to inspect Pipeline Template media storage.', 503);
  }
  if (!existing.data) {
    ...
    if (uploaded.error) {
      ...
      if (!raced.data) {
        throw new DeveloperAssetRegistryCommandError('Unable to store Pipeline Template artwork.', 503);
      }
    }
  }
  const record: PipelineTemplateAssetRecord = {
    asset_id: assetId,
    storage_bucket: DEVELOPER_ASSET_STORAGE_BUCKET,
  ...
  if (error) {
    throw new DeveloperAssetRegistryCommandError('Unable to record Pipeline Template artwork ownership.', 503);
  }
...
const visit = async (
  ...
  if (depth > MAX_TRAVERSAL_DEPTH) {
    throw new DeveloperAssetRegistryCommandError('Pipeline Template artwork is nested too deeply.', 400);
  }

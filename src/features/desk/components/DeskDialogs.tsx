...
export function DeskDialogs(props: DeskDialogsProps) {
  ...
  return <>
    ...
    <AlertDialog open={props.dirtyCloseRequested} onOpenChange={props.onDirtyCloseOpenChange}><AlertDialogContent className="border-[var(--cf-border-strong)] bg-[var(--cf-surface)] text-[var(--cf-text)]"><AlertDialogHeader><AlertDialogTitle>Close {props.dirtyToolName ?? 'Design'} with unsaved changes?</AlertDialogTitle><AlertDialogDescription className="leading-6 text-[var(--cf-text-muted)]">Save your changes before closing if you want to keep them. Closing will leave your saved work unchanged.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Keep editing</AlertDialogCancel><AlertDialogAction onClick={props.onConfirmDirtyClose}>Close {props.dirtyToolName ?? 'Design'}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </>;
}

...
interface DeskDialogsProps {
  ...
  dirtyCloseToDesk?: boolean;
  dirtyToolName?: string;
...
export function DeskDialogs(props: DeskDialogsProps) {
  const freshSetDescriptionId = useId();
  const freshSetDesignDescriptionId = useId();

  return <>
    <Dialog open={props.createOpen} onOpenChange={props.onCreateOpenChange}><DialogContent className={styles.createDialog}><DialogHeader><DialogTitle>Start a new Set</DialogTitle><DialogDescription>Start empty, design a card layout, or customize a published starter. Your starter copy is independent; the published original stays unchanged.</DialogDescription></DialogHeader><div className={styles.createChoices}>
      <button type="button" className={styles.createChoice} aria-label="Fresh Set" aria-describedby={freshSetDescriptionId} onClick={() => props.onCreateWork(false)}><span className={styles.createChoiceVisual}><FolderPlus aria-hidden="true" /></span><span><strong>Fresh Set</strong><small id={freshSetDescriptionId}>Start with no cards. Add a layout and generate cards when you are ready.</small></span></button>
      <button type="button" className={styles.createChoice} aria-label="Fresh Set + Design" aria-describedby={freshSetDesignDescriptionId} onClick={() => props.onCreateWork(true)}><span className={styles.createChoiceVisual}><Pencil aria-hidden="true" /></span><span><strong>Fresh Set + Design</strong><small id={freshSetDesignDescriptionId}>Create a Set and open the card layout editor. Design once, then use that layout for many cards.</small></span></button>
    ...
    <AlertDialog open={props.dirtyCloseRequested} onOpenChange={props.onDirtyCloseOpenChange}><AlertDialogContent className="border-[var(--cf-border-strong)] bg-[var(--cf-surface)] text-[var(--cf-text)]"><AlertDialogHeader><AlertDialogTitle>{props.dirtyCloseToDesk ? 'Return to Desk with unsaved changes?' : `Close ${props.dirtyToolName ?? 'Design'} with unsaved changes?`}</AlertDialogTitle><AlertDialogDescription className="leading-6 text-[var(--cf-text-muted)]">{props.dirtyCloseToDesk ? 'Returning to Desk closes the open tools and discards their unsaved drafts. Your saved Sets, cards, and Templates stay unchanged.' : 'Save your changes before closing to keep them. Closing will leave your saved work unchanged.'}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Keep editing</AlertDialogCancel><AlertDialogAction onClick={props.onConfirmDirtyClose}>{props.dirtyCloseToDesk ? 'Discard & return to Desk' : `Close ${props.dirtyToolName ?? 'Design'}`}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </>;
}

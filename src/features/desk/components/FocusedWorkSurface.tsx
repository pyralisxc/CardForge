...
export function FocusedWorkSurface(props: FocusedWorkSurfaceProps) {
  ...
  return <div className={styles.focusSurface} data-desk="focused" data-focus-transition="set-to-artifacts" data-artifact-focused={artifactFocused}>
    <section className={styles.focusWorkspace} data-desk-set-board data-artifact-focused={artifactFocused} aria-label={props.item.name}>
      {props.localSetId ? <>
        {!artifactFocused ? <><SetTemplates cards={props.focusedCards} onDesign={props.onDesignTemplate} />
        <div className={styles.contentToolbar}>
          ...
          <span className={styles.gestureHint}>Drag to move · Hold on touch · Pinch or Ctrl+scroll to zoom</span>
          <details className={styles.setOrganization}>
            <summary>Organize · {props.selectedCards.length ? `${props.selectedCards.length} selected` : `${props.visibleCards.length} card${props.visibleCards.length === 1 ? '' : 's'}`}</summary>

    ...
    <section className={styles.focusWorkspace} data-desk-set-board data-artifact-focused={artifactFocused} aria-label={props.item.name}>
      {props.localSetId ? <>
        {!artifactFocused ? <>
          ...
          {props.selectedCards.length ? <div className={`${styles.selectionBar} border-t border-[var(--cf-border-subtle)] bg-[var(--cf-surface-inset)] px-2 py-2`} role="toolbar" aria-label="Selection actions">
            <span role="status">{props.selectedCards.length === 1 ? `${getCardTitle(props.selectedCards[0]!, 0)} selected` : `${props.selectedCards.length} Artifacts selected`}{props.selectionScope.hidden ? ` · ${props.selectionScope.hidden} hidden by current filters; actions apply to the full selection` : ''}</span>
            <Button type="button" size="sm" variant="outline" onClick={props.onReviseSelected}><Pencil className="mr-1.5 h-4 w-4" />Edit selected</Button>
            ...
            <Button type="button" size="sm" variant="outline" onClick={props.onDuplicateSelected}><Copy className="mr-1.5 h-4 w-4" />Duplicate</Button>
            <Button type="button" size="sm" variant="ghost" onClick={props.onDeleteSelected}><Trash2 className="mr-1.5 h-4 w-4" />Remove</Button>

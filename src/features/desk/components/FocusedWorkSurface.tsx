...
import { ArrowDown, ArrowUp, Boxes, Copy, Layers3, LayoutGrid, MoreHorizontal, Pencil, Search, Sparkles, Tag, Trash2, TriangleAlert, X } from 'lucide-react';
...
export interface FocusedWorkSurfaceProps {
  ...
  availableFields: ReflectiveFieldFacet[];
  reflectiveGroupings: ReflectiveOrganization['groupings'];
      ...
      {props.localSetId ? <>
        {!artifactFocused ? <>
          ...
          {props.selectedCards.length ? <div
            ...
            style={{ flex: '0 0 auto' }}
          >
            <span role="status" className={styles.selectionCount}>{props.selectedCards.length === 1 ? `${getCardTitle(props.selectedCards[0]!, 0)} selected` : `${props.selectedCards.length} Artifacts selected`}{props.selectionScope.hidden ? ` · ${props.selectionScope.hidden} hidden by current filters; actions apply to the full selection` : ''}</span>
            ...
            <div className={styles.selectionDesktopActions}>
            ...
            <Popover><PopoverTrigger asChild><Button type="button" size="icon" variant="ghost" className={styles.selectionMore} aria-label="More selection actions"><MoreHorizontal aria-hidden="true" /></Button></PopoverTrigger>
              <PopoverContent align="end" className={styles.selectionActionPanel} aria-label="More selection actions">
                ...
                {props.selectedCard ? <div className={styles.selectionActionRow}><Button type="button" size="sm" variant="outline" disabled={props.selectedCardIndex <= 0} onClick={() => props.onReorderSelected('earlier')}><ArrowUp className="h-4 w-4" />Move earlier</Button><Button type="button" size="sm" variant="outline" disabled={props.selectedCardIndex < 0 || props.selectedCardIndex >= props.focusedCards.length - 1} onClick={() => props.onReorderSelected('later')}><ArrowDown className="h-4 w-4" />Move later</Button></div> : null}
                {props.otherSets.length ? <div className={styles.selectionActionRow}><Select value={props.moveTargetId} onValueChange={props.onMoveTargetChange}><SelectTrigger className={styles.moveSelect} aria-label="Move selected card to Set"><span className="truncate">Move to {props.otherSets.find((set) => set.id === props.moveTargetId)?.name ?? 'Set'}</span></SelectTrigger><SelectContent>{props.otherSets.map((set) => <SelectItem key={set.id} value={set.id}>{set.name}</SelectItem>)}</SelectContent></Select><Button type="button" size="sm" variant="outline" onClick={props.onMoveSelected}>Move</Button></div> : null}
                <div className={styles.selectionActionRow}><Button type="button" size="sm" variant="outline" onClick={props.onDuplicateSelected}><Copy className="mr-1.5 h-4 w-4" />Duplicate</Button><Button type="button" size="sm" variant="ghost" onClick={props.onDeleteSelected}><Trash2 className="mr-1.5 h-4 w-4" />Remove</Button></div>

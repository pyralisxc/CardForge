...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, CreditCard, FolderPlus, Hand, HardDrive, LayoutGrid, Link2, Loader2, Search, ShieldCheck, Sparkles, X } from 'lucide-react';
    ...
    <section className={styles.workSurface} data-grid={props.showGrid} aria-labelledby="desk-open-work-heading">
      ...
      <div className={styles.deskToolbar}>
        ...
        <div className={styles.spatialControls} aria-label="Desk positioning">
          ...
          <Button type="button" size="sm" variant={arrangeMode ? 'secondary' : 'ghost'} aria-pressed={arrangeMode} onClick={() => setArrangeMode((current) => !current)}><Hand className="mr-1 h-4 w-4" aria-hidden="true" />Arrange</Button>
        </div>
      </div>
      ...
      {props.isLoading && !props.workItemsCount ? <div className={styles.emptyDesk}><div className={styles.emptyDeskInner}><Loader2 className="animate-spin" aria-hidden="true" /><strong>Preparing your desk</strong></div></div> : props.visibleWork.length ? <div
        ...
        onPointerDown={(event) => props.beginMarquee(event, arrangeMode)}
        onPointerMove={props.moveMarquee}
        onPointerUp={props.endMarquee}
        onPointerCancel={props.endMarquee}
      >
